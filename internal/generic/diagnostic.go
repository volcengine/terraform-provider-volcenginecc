...
func ResourceNotFoundAfterWriteDiag(err error) diag.Diagnostic {
	return diag.NewErrorDiagnostic(
		"Resource Not Found After Creation or Update",
		fmt.Sprintf("After creating or updating the resource and attempting to read the resource, the API returned a resource not found error. This is typically an error with the Terraform resource implementation. Original Error: %s", err.Error()),
	)
}
...
func ServiceOperationEmptyResultDiag(service string, operation string) diag.Diagnostic {
	return diag.NewErrorDiagnostic(
		"SDK Go Service Operation Empty Response",
...
func ServiceOperationErrorDiag(service string, operation string, err error) diag.Diagnostic {
	return diag.NewErrorDiagnostic(
		"SDK Go Service Operation Unsuccessful",
...
func ServiceOperationWaiterErrorDiag(service string, operation string, err error) diag.Diagnostic {
	return diag.NewErrorDiagnostic(
		"SDK Go Service Operation Incomplete",
